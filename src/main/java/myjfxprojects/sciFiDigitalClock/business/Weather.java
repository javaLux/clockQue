...
public class Weather {
	...
	private Weather() {
		...
		/*
		 * add ChangeListener to the BooleanProperty value which says if Internet connection is up or down
		 * if this value changed from false to true -> than start once a new time line to fetch weather data again
		 * IMPORTANT because if the application started only the periodically time line and these is fetching the
		 * weather data every minute -> this means the weather data can not updated one minute long
		 */
		DataBean.isInternetUp.addListener((observable, oldValue, newValue) -> {
			
			if(oldValue.booleanValue() == false) {
				
				// the default value from variable "isInternetUp" is false, IMPORTANT if Application starts and the scheduled service to check the
				// Internet connectivity will execute -> than will this value changed to true if Internet is available and fetching weather data begins or
	...
	/**
	 * Method starts a time line who periodically fetch the current weather data (dependent on the duration value).
	 */
	public void startPeriodicallyFetchWeatherData() {
		this.timelineWeatherData.setCycleCount(Timeline.INDEFINITE);
		
		this.timelineWeatherData.getKeyFrames().add(new KeyFrame(Duration.minutes(this.durationWeatherTimeline),
        		event -> {
	 ...
	 * location object in database.
	 * Farther the tool tip from digital clock will be updated with new time zone info
	 * and the date info's will be updated.
	 * Last but not least, the current time shift in hours will be add to the digital clock tool tip.
	 * 
	...
	private void getWeatherData() {
		...
		if(DataBean.isInternetUp.get()) {
			...
			try {
//				System.out.println("Weather URL: '" + this.dataBean.getAPI_WEATHER_URL() + "'");
				URL url = new URL(this.dataBean.getAPI_WEATHER_URL());

				HttpsURLConnection connection = (HttpsURLConnection) url.openConnection();
				
				// give incoming weather data to BufferReader to read data
...
//				System.out.println(jsonResponseString + "\n");			
				...
				if(! this.mapWeatherData.isEmpty()) {
					...
					// IMPORTANT:	If Weather icon with associated tool tip and the temperature with associated tool tip
					//				are showing will managed in class "ControlDigitalClock" for detail in the Time line of
					//				the digital clock. These time line will check every second the current state of the
					//				Internet connectivity and if waiting or error symbol are showing. Is not that case, the
					...
					// set correct weather data, e.g. temperatureCurrent, temperature unit and weather description and so on in GUI
					this.dataBean.getDigitalClockFXMLcontroller().getLblCurrentTemp().setText(this.temperatureCurrent);
					this.dataBean.getDigitalClockFXMLcontroller().getTextTempUnit().setText(this.dataBean.getCurrentTempUnit().getTempUnit());
					...
					if(! this.weatherConIconPath.isEmpty()) {
						...
						this.dataBean.getDigitalClockFXMLcontroller().setImageViewWeather(new Image(this.weatherConIconPath));
						// set tool tips for weather icon
						this.weatherIconToolTip.setWeatherText(this.weatherDescription, this.dateTimeSunrise, this.dateTimeSunSet);
					}
					else {
...
//					System.out.println("Weather Description: " + this.weatherDescription + "\n");
				...
				LOGGER.error("Failed to get weather data from URL: '" + this.dataBean.getAPI_WEATHER_URL() + "'", ex);
	...
	/**
	 * Method fetch the time zone for the current location to convert sunrise and sunset times correctly.
	 * 
	 * @return	->	[String] 	with the current time zone (for e.g. Europe/Berlin)
	 * 							or an empty String if no time zone was found.
	 */
	...
	private String fetchCurrentTemp() {
		...
		if(this.mapWeatherData.containsKey(CURRENT_KEY)) {
			...
			if(tempMap.containsKey(TEMP_KEY)) {
				...
				// here we must use type long, because in case that we must rounded the value from
	...
	private String fetchDailyMinTemp() {
		...
		if(this.mapWeatherData.containsKey(DAILY_KEY)) {
			List<Map<String, Object>> dailyForcastValueList = (List<Map<String, Object>>) this.mapWeatherData.get(DAILY_KEY);
			
			if(! dailyForcastValueList.isEmpty()) {
				// get the current day from daily list -> this is always the first element in the list
	 ...
	 * 
	 * @return	->	an integer with the weather condition id or the value 0 if
	 * 				no weather condition id available.
	 * 
	 * @throws	->	throws an NumberFormatException if available id can not parse to an integer.
	 */
	private Integer fetchWeatherConID() {
		...
		if(this.mapWeatherData.containsKey(CURRENT_KEY)) {
			...
			if(tempMapCurrent.containsKey(WEATHER_KEY)) {
				ArrayList<Map<String, Object>> tempList = (ArrayList<Map<String, Object>>) tempMapCurrent.get(WEATHER_KEY);
	 ...
	 * 
	 * @return	->	an empty string if no weather description available
	 * 				or the weather description as string.
	 */
	...
	/**
	 * Method fetch UNIX time stamps from weather map,
	 * for the current day. Dependent on the given time stamp key.
	 * 
		...
		if(this.mapWeatherData.containsKey(CURRENT_KEY)) {
			...
			if(tempMapCurrent.containsKey(timeStampKey)) {
				try {
					...
					LOGGER.error("Failed to convert time stamp of the current day, from string to long.\nArgs: 'timeStampKey': " + timeStampKey
							+ " Time stamp: '" + tempMapCurrent.get(timeStampKey) + " '", ex);
				}
			}
		}
	 ...
	 * Shows the waiting Symbol instead of the weather icon and changed the tool tip.
	 */
	...
	/**
	 * Method is used when an exception will thrown by querying the current weather data from OpenWeather API.
	 * Shows the error Symbol instead of the weather icon and changed the tool tip.
	 */
